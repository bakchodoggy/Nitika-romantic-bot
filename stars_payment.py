...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # Or ["https://gems-miniapp.vercel.app"] for more security
...
async def user_endpoint(request: Request):
    ...
    # Dummy/example response — replace with real values if you have a database
    return {
        "status": "success",
        "user_id": user_id,
...
async def buy_gems(request: Request):
    data = await request.json()
    # Dummy logic — replace with real purchase logic if needed
